...
- **Streaming:** Prioritize streaming recognition to ensure Jarvis reacts while the user is speaking.
...
- **No Background Loops for Actions:** Do not create persistent `asyncio` loops (like the old `_volume_control_loop`) for gesture-based actions. All actions should be event-driven through `StateMachine.on_gesture_detected()`.
...
- When adding a new voice command: add to `_local_route()` mapping AND `is_valid_command()` keywords AND `tools_schema` if LLM should know about it.
...
- All colors must come from `jarvis/gui/theme.py` constants — no hardcoded hex values in component files.
